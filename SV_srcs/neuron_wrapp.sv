...
module neuron_wrapp(
    ...
    output logic [$clog2(`L4_neurons)-1:0] predicted_class,
   ...
   dense_layer #(.weight_file("l4_weights.hex"),.weights(`L4_weights)) 
   ...
   argMAX #(.num_classes(`L4_neurons),.data_width(`dense_width))
   classifier_out(.data_in(m_axis_tdata_out),.predicted_class(predicted_class)
   );
   
   assign prediction_valid = m_axis_tvalid_out;
endmodule